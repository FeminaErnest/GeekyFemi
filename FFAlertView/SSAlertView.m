...
- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)AlertDelegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle
{
    ...
    if (self) {
        ...
        self.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
         ...
         UIImage *AlertBg = [UIImage imageNamed:@"1_small.png"];
        
        if ([AlertBg respondsToSelector:@selector(resizableImageWithCapInsets:)])
            AlertBg = [[UIImage imageNamed: @"1_small.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(40.0, 25.0, 25.0, 25.0)];
        else
            AlertBg = [[UIImage imageNamed: @"1_small.png"] stretchableImageWithLeftCapWidth: 25 topCapHeight: 40];

        
        UIImage *CancelBtnImg = [UIImage imageNamed:@"2_small.png"];
        UIImage *OtherBtnImg = [UIImage imageNamed:@"2_small.png"];
        ...
        if (message)
        {
            ...
            MessageLbl.font = [UIFont boldSystemFontOfSize:14.0];
        ...
        if (cancelButtonTitle && otherButtonTitle)
        {
            ...
            [CancelBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:14.0]];
            ...
            [OtherBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:14.0]];
...
- (void)onBtnPressed:(id)sender
{
    ...
    btnTitleText = [[button titleLabel] text];
    
    NSLog(@"btnTitleText is:%@",btnTitleText);
    
    int button_index = button.tag-1000;
    
    if ([delegate respondsToSelector:@selector(SSAlertView:clickedButtonAtIndex:)])
        [delegate SSAlertView:self clickedButtonAtIndex:button_index];
...
{
    NSLog(@"getting btn index as :%d corresponding text as:%@",buttonIndex,btnTitleText);
    return btnTitleText;
}
